...
# PyPDF
pypdf_imported = True
try: from pypdf import PdfReader,filters
...
class ChloeAI:
    ...
    def __init__(self, database_location : str | None = None, database_name : str = 'datumbazo', maximum_pixels : int = 10_000_000_000, histogram_ratio_precision : int = 6, allow_truncating_images : bool = False, pdf_max_array_out_stream_len : int = 100_000_000, pdf_max_declared_stream_len : int = 100_000_000, pdf_jbig2_max_out_len : int = 75_000_000, pdf_lzw_max_out_len : int = 75_000_000, pdf_zlib_max_out_len : int = 75_000_000, pdf_zlib_recovery_in_len : int = 5_000_000, pdf_flate_max_columns : int = 250_000, pdf_flate_max_row_len : int = 4_000_000, pdf_flate_max_buffer_size : int = 75_000_000, pdf_run_len_max_out_len : int = 75_000_000, crintum_obfuscation : bool = False, chloe_vocalization : bool = False, use_audio_wakeup_buffer : bool = False, audio_wakeup_buffer : int = 10, allow_autoclear_terms : bool = False, auto_update_database : bool = False, keep_db_if_no_connection : bool = True, clear_search_update : bool = False, updateCFB_progress_display : bool = False, corrupted_zip_check : bool = True):
        ...
        self.acceptable_chars = set(printable[:-5])
        ...
        for item in tuple(listdir(self.db_path)):
            if isdir(f'{self.db_path}/{item}') and item != '_terms_searched':
    ...
    def updateCFB(self, keep_db_if_no_connection : bool = True, clear_search_update : bool = False, terminal_progress_display : bool = False) -> None:
        #CFB stands for Chloe Felina Basics
        ...
        def getBasicInfo(item_path : str) -> tuple:
            ...
            try: metadata.append(getModifiedDate(item_path)[4:])
            except Exception: metadata.append(None)
            try: metadata.append(getModifiedDate(item_path)[4:])
            except Exception: metadata.append(None)
            try: metadata.append(str(getSizeOfItem(item_path)))
            except Exception: metadata.append(None)
            ...
            if keep_db_if_no_connection:
                ...
                for db_name in iterator:
                    if not exists(self.path_pointer[db_name]):
                        redact_dbs.append(db_name)
            if len((redact_dbs := tuple(redact_dbs))):
                change_done = True
            for redact_db in redact_dbs:
                    ...
                    if not item in testing_set:
                        ...
                        if not '.' in item:
                            ...
                            match (extension := item[item.rfind('.')+1:]):
                                ...
                                case 'bin':
                                    try:
                                        if not self.archive_bin_data(f'{ref_path}/{item}',db_name):
                                            self.archive_alia_data(f'{ref_path}/{item}',db_name)
                                    except Exception:
                                        self.archive_alia_data(f'{ref_path}/{item}',db_name)
                                case 'docx':
        ...
        if reference_directory in self.paths_in_db or reference_directory.lower().endswith('.gdb') or reference_directory == self.db_path:
            ...
            for name in tuple(listdir(reference_directory)):
                if isfile(f'{reference_directory}/{name}'):
                    if (temp_name := name.lower()).endswith('.shp'):
                        items[name] = 'SHP'
                    elif temp_name.endswith('.txt'):
                        items[name] = 'TXT'
                    elif temp_name.endswith('.pdf'):
                        items[name] = 'PDF'
                    elif temp_name.endswith('.docx'):
                        items[name] = 'DOC'
                    elif temp_name.endswith('.bin'):
                        items[name] = 'BIN'
            ...
            if len(items):
                ...
                for name in names:
                    match items[name]:
                        ...
                        case 'BIN':
                            try:
                                if not self.archive_bin_data(f'{reference_directory}/{name}',archive_db_name):
                                    self.archive_alia_data(f'{reference_directory}/{name}',archive_db_name)
                            except Exception:
                                self.archive_alia_data(f'{reference_directory}/{name}',archive_db_name)
                        case 'PDF':
    ...
    def archive_bin_data(self, bin_path : str, archive_db_name : str) -> bool:

        if (baseline_metadata := getBaselineMetadata(bin_path)) is None:
            return False
        ...
        for n in range(len(bin_lines)):
            try:
                bin_lines[n] = bin_lines[n].decode('latin-1')
            except Exception:
                bin_lines[n] = None

        while None in bin_lines:
            bin_lines.remove(None)

        extract_texted = ()

        if len((bin_lines := tuple(bin_lines))):
            extracted_text = tuple(["".join([char for char in bin_line if char in acceptable_chars]) for bin in bin_lines])

        bin_nums = [lines[n][x] for n in range(len(lines)) for x in range(len(lines[n]))]

        del lines

        val_counter = 1 ; bin_num_order_count = []
        bin_num_order = [(current_num := bin_nums[0])]

        for n in range(1,len(bin_nums)):
            if bin_nums[n] != current_num:
                bin_num_order_count.append(val_counter)
                val_counter = 1
                bin_num_order.append(bin_nums[n])
                current_num = bin_nums[n]
            else:
                val_counter += 1

        del current_num

        if len(bin_num_order) > len(bin_num_order_count):
            bin_num_order_count.append(val_counter)
            bin_num_order.append(bin_nums[-1])

        del val_counter

        replacers = {'10':'a','12':'b','13':'c','14':'d','15':'e','16':'f','17':'g','18':'h','19':'i','20':'j','21':'k','23':'l','24':'m','25':'n','26':'o','27':'p','28':'q','29':'r','30':'s','31':'t','32':'u','34':'v','35':'w','36':'x','37':'y','38':'z','39':'A','40':'B','41':'C','42':'D','43':'E','45':'F','46':'G','47':'H','48':'I','49':'J','50':'K','51':'L','52':'M','53':'N','54':'O','56':'P','57':'Q','58':'R','59':'S','60':'T','61':'U','62':'V','63':'W','64':'X','65':'Y','67':'Z'}

        nums_keys = tuple(replacers.keys())

        lines = []

        for line in tuple([str(bin_num_order[n]) if bin_num_order_count[n] == 1 else f'{bin_num_order[n]}*{bin_num_order_count[n]}' for n in range(len(bin_num_order))]):
            for nums in nums_keys:
                line = line.replace(nums,replacers.keys())
            lines.append(line)

        del bin_num_order ; del bin_num_order_count
        ...
        for n in range(1,len(lines)):
            if '*' in lines[n] or '*' in simplified_lines[-1]:
                simplified_lines.append(lines[n])
            else:
                simplified_lines[-1] = f'{simplified_lines[-1]}{lines[n]}'

        lines = tuple(simplified_lines)

        simplified_lines = []

        if len(lines) % 2 == 0:
            simplified_lines = tuple([f'{lines[n]}+{lines[n+1]}' for n in range(0,len(lines),2)])
        else:
            simplified_lines = [f"{lines[n]}+{lines[n+1]}" for n in range(0,len(lines)-1,2)]
            simplified_lines.append(lines[-1])
            simplified_lines = tuple(simplified_lines)

        del lines

        val_counter = 1 ; bin_num_order_count = []
        bin_num_order = [(current_num := simplified_lines[0])]

        for n in range(1,len(simplified_lines)):
            if simplified_lines[n] != current_num:
                bin_num_order_count.append(val_counter)
                val_counter = 1
                bin_num_order.append(simplified_lines[n])
            else:
                val_counter += 1

        del simplified_lines ; del current_num

        if len(bin_num_order) > len(bin_num_order_count):
            bin_num_order_count.append(val_counter)
            bin_num_order.append(simplified_lines[-1])

        del val_counter

        abridged_lines = [str(bin_num_order[n]) if bin_num_order_count[n] == 1 else f'{bin_num_order[n]}^{bin_num_order_count[n]}' for n in range(len(bin_num_order))]
        replacers_2 = {'10':"_",'20':'-','30':'!','40':'?','50':'@','60':'#','70':'$','80':'=','90':'&'}
        chars = tuple(replacers_2.keys())

        del bin_num_order ; del bin_num_order_count

        for n in range(len(abridged_lines)):
            for char in chars:
                abridged_lines[n] = abridged_lines[n].replace(char,replacers_2[char])

        del replacers_2 ; del chars

        nums = [] ; starting_index = 0 ; starting_prefix = "" ; combo = []

        for abridged_line in (abridged_lines := tuple(abridged_lines)):
            starting_index += 1
            if '+' in abridged_line:
                starting_prefix = abridged_line[:abridged_line.find('+')+1]
                combo.append(abridged_line[abridged_line.find('+')+1:])
                break

        for n in range(starting_index,len(abridged_lines)):
            if abridged_lines[n].startswith(starting_prefix) and starting_prefix != "":
                combo.append(abridged_lines[n])
            elif starting_prefix == "":
                if '+' in abridged_lines[n]:
                    starting_prefix = abridged_lines[n][:abridged_lines[n].find('+')+1]
                    combo.append(abridged_lines[n][abridged_lines[n].find('+')+1:])
                else:
                    nums.append(abridged_lines[n])
            else:
                nums.append("%s%s" % (starting_prefix,"|".join(combo)))
                combo = []
                starting_prefix = ""
                if '+' in abridged_lines[n]:
                    combo.append(abridged_lines[n][abridged_lines[n].find('+')+1:])
        ...
        del starting_prefix ; del abridged_lines ; del combo

        nums = tuple(nums)

        if not 0 in (counters := (len(extract_texted),len(nums))):
            file_name = bin_path[bin_path.rfind('/')+1:]
            nomin_kalvamortum = "%s_%s" % (file_name[:file_name.rfind('.')],file_name[file_name.rfind('.')+1:])
            del file_name
            mkdir(f'{self.db_path}/{archive_db_name}/{nomin_kalvamortum}')
            if counters[0]:
                with open(f'{self.db_path}/{archive_db_name}/{nomin_kalvamortum}/bin_extracted_text.txt','w',encoding='utf-8') as tf:
                    tf.write(extracted_text[0])
                    for n in range(1,len(extracted_text)):
                        tf.write(f'\n{extracted_text[n]}')
            del extracted_text
            if counters[1]:
                with open(f'{self.db_path}/{archive_db_name}/{nomin_kalvamortum}/bin_nums.txt','w',encoding='utf-8') as tf:
                    tf.write(nums[0])
            ...
            if not exists((_metadata := f'{self.db_path}/{archive_db_name}/_metadata.txt')):
                try:
                    with open(_metadata,'w',encoding='utf-8') as tf:
                        tf.write(f'{nomin_kalvamortum}|BIN|{baseline_metadata}|{counters[0]}|{counters[1]}')
                except UnicodeEncodeError:
                    with open(_metadata,'w',encoding='latin-1') as tf:
                        tf.write(f'{nomin_kalvamortum}|BIN|{baseline_metadata}|{counters[0]}|{counters[1]}')
            else:
                try:
                    with open(_metadata,'a',encoding='utf-8') as tf:
                        tf.write(f'\n{nomin_kalvamortum}|BIN|{baseline_metadata}|{counters[0]}|{counters[1]}')
                except UnicodeEncodeError:
                    with open(_metadata,'a',encoding='latin-1') as tf:
                        tf.write(f'\n{nomin_kalvamortum}|BIN|{baseline_metadata}|{counters[0]}|{counters[1]}')
    ...
    def searchQuery(self, entry_string : str, check_type : str | tuple[str] | list[str] | set[str] = 'any', include_entity_name : bool = True, entity_names_only : bool = False, return_tuple : bool = False, max_line_concat : int = 3, save_found_matches : bool = True, save_to_file : bool = False, output_file_type : str = 'excel', output_location : str | None = None, output_name : str | None = None, overwrite_existing_output : bool = False, csv_field_size_limit : int = 131_072, csv_delimiter : str = ',', overwrite_saved_found_matches : bool = False, terminal_progress_display : bool = False) -> tuple[str] | None:
        ...
        entry_string = " ".join(["".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',item)) if segment]) for item in temp_str]).lower()
        ...
        testing_entry_string = " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',entry_string)) if segment]).lower()
        ...
        if not exists(f'{self.db_path}/_terms_searched'):
            ...
            if ' ' in entry_string:
                ...
                for previous_search in tuple([txt_file[:txt_file.rfind(".")] for txt_file in tuple(listdir(f'{self.db_path}/_terms_searched'))]):
                    if previous_search in test_name_str:
                        words[previous_search[:previous_search.rfind('$')]] = True
                if len((found_words := tuple([word for word in tuple(words.keys()) if words[word]]))):
                                ...
                                for found_word in found_words:
                                    with open(f'{self.db_path}/_terms_searched/{found_word}$names.txt',encoding='utf-8') as tf:
                                        while True:
                                            ...
                                            if '.' in (file_name := entity[entity.rfind('\\')+1:].lower()):
                                                ...
                                                if testing_entry_string in " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name)) if segment]):
                                                    found_name_matches.add(entity)
                            case 'txt' | 'bin' | 'pdf' | 'shp' | 'gdb':
                                extension = f'.{check_type}'
                                for found_word in found_words:
                                    with open(f'{self.db_path}/_terms_searched/{found_word}$names.txt',encoding='utf-8') as tf:
                                        while True:
                                            entity = tf.readline()
                                ...
                                for found_word in found_words:
                                    with open(f'{self.db_path}/_terms_searched/{found_word}$names.txt',encoding='utf-8') as tf:
                                        while True:
                                            ...
                                            entity = entity.rstrip('\n')
                                            if '.' in (file_name := entity[entity.rfind('\\')+1:].lower()):
                                                if not file_name[file_name.rfind('.')+1:] in self.valid_extensions:
                                                    if testing_entry_string in " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name[:file_name.rfind('.')])) if segment]):
                                                        found_name_matches.add(entity)
                                            else:
                                                if testing_entry_string in " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name)) if segment]):
                        ...
                        for found_word in found_words:
                            with open(f'{self.db_path}/_terms_searched/{found_word}$names.txt',encoding='utf-8') as tf:
                                while True:
                                    ...
                                    if not entity:
                                        break
                                    entity = entity.replace('\\','/').rstrip('\n')
                                    if '.' in (file_name := entity[entity.rfind('/')+1:].lower()):
                                        if file_name[file_name.rfind('.')+1:] in check_type:
                                            if testing_entry_string in " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name[:file_name.rfind('.')])) if segment]):
                                                found_name_matches.add(entity)
                ...
                for previous_search in tuple([txt_file[:txt_file.rfind(".")] for txt_file in tuple(listdir(f'{self.db_path}/_terms_searched')) if txt_file.endswith('$names.txt')]):
                    if test_str == previous_search:
                        if isinstance(check_type,str):
                                ...
                                case 'img':
                                    with open(f'{self.db_path}/_terms_searched/{previous_search}.txt',encoding='utf-8') as tf:
                                        while True:
                                            ...
                                            entity = entity.rstrip('\n')
                                            if '.' in (file_name := entity[entity.rfind('\\')+1:].lower()):
                                                if file_name[file_name.rfind('.'):] in self.accepted_image_extensions:
                                                    if testing_entry_string in ' '.join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name[:file_name.rfind('.')])) if segment]):
                                                        found_name_matches.append(entity)
                                case 'alia':
                                    with open(f'{self.db_path}/_terms_searched/{previous_search}.txt',encoding='utf-8') as tf:
                                        while True:
                                            ...
                                            if not entity:
                                                break
                                            entity = entity.rstrip('\n')
                                            if not '.' in (file_name := entity[entity.rfind('\\')+1:].lower()):
                                                if testing_entry_string in ' '.join([segment for segment in tuple(re.split(r'[a-zA-Z0-9]+',file_name)) if segment]):
                                                    found_name_matches.append(entity)
                            ...
                            for item in ('all','any','every'):
                                if item in check_type:
                                    check_type.remove(item)
                            if not len(check_type):
                                if return_tuple:
                                    return ()
                                return None
                            set_image_types = set(self.image_types)
                            with open(f'{self.db_path}/_terms_searched/{previous_search}.txt',encoding='utf-8') as tf:
                                while True:
                                    entity = tf.readline()
                                    if not entity:
                                        break
                                    entity = entity.rstrip('\n')
                                    if '.' in (file_name := entity[entity.rfind('\\')+1:]):
                                        if (extension := file_name[file_name.rfind('.')+1:].lower()) in check_type:
                                            match extension:
                                                case 'txt' | 'bin' | 'pdf' | 'docx' | 'shp' | 'gdb':
                                                    if testing_entry_string in " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name[:file_name.rfind('.')])) if segment]):
                                                        found_name_matches.append(entity)
                                                case _:
                                                    if extension in set_image_types:
                                                        if testing_entry_string in " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name[:file_name.rfind('.')])) if segment]):
                                                            found_name_matches.append(entity)
                                                    elif 'alia' in check_type:
                                                        if testing_entry_string in " ".join([segment for segment in tuple(re.split(r'[^a-zA-Z0-9]+',file_name[:file_name.rfind(".")])) if segment]):
                ...
                if len((found_words := tuple([word for word in tuple(words.keys()) if words[word][0]]))):
                    ...
                    if isinstance(check_type,str):
                        if (check_type := check_type.lower()) == 'docx':
                            ...
                            check_type = 'img'
                        anything = {'any','all','every'}
                        line_count_contents = {found_word : 0 for found_word in found_words}
                                    ...
                                    with open(f'{self.db_path}/_terms_searched/{found_word}$contents.txt',encoding='utf-8') as tf:
                                        while True:
                                            ...
                                            if not entity:
                                                break
                                            counter += 1
                                    line_count_contents[found_word] = counter
                                ...
                                if words[found_word][1]:
                                    ...
                                    with open(f'{self.db_path}/_terms_searched/{found_word}$contents.txt',encoding='utf-8') as tf:
                                        while True:
                                            ...
                                            counter += 1
                                    line_count_contents[found_word] = counter
                        found_name_matches = list(found_name_matches)
                        least_items = "NOTHING"
                        min_num = 10_000_000_000
                        for found_word in found_words:
                            if min_num > line_count_contents[found_word]:
                                least_items = found_word[:]
                                min_num = line_count_contents[found_word]
                        relevant_entities = []
                        with open(f'{self.db_path}/_terms_searched/{found_word}$contents.txt',encoding='utf-8') as tf:
                            while True:
                                entity = tf.readline()
                                if not entity:
                                    break
                                relevant_entities.append(entity.rstrip('\n'))
                        grouped_entities = {}
                        for relevant_entity in (relevant_entities := tuple(relevant_entities)):
                            if (zip_name := self.crintum_pointer[relevant_entity[:relevant_entity.rfind("\\")].replace('\\','/')]) in grouped_entities.keys():
                                grouped_entities[zip_name].add(relevant_entity[relevant_entity.rfind('\\')+1:])
                            else:
                                grouped_entities[zip_name] = {relevant_entity[relevant_entity.rfind("\\")+1:]}
                        if tqdm_imported:
                            iterator = tqdm(tuple(grouped_entities.keys()),disable = not terminal_progress_display, desc = f"Searching for instances of {entry_string}")
                        else:
                            iterator = tuple(grouped_entities.keys())
                        if not check_type in anything:
                            for used_name in iterator:
                                current_grouping = grouped_entities[used_name]
                                with ZipFile(f'{self.db_path}/{used_name}.zip') as zf:
                                    if '_metadata.txt' in (items := zf.namelist()) and check_type != 'gdb':
                                        with zf.open('_metadata.txt') as tf:
                                            while True:
                                                line = tf.readline()
                                                if not line:
                                                ...
                                                if "%s.%s" % (line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]) in current_grouping:
                                                    if line[1].lower() == check_type:
                                                        match line[1]:
                                                            case 'TXT':
                                                                if isQueryMatchGewurah(entry_string,tuple(zf.open(f'_txt_files/{line[0]}.txt').readlines()),max_line_concat):
                                                                    found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                                                            case 'BIN':
                                                                if isQueryMatchHod(entry_string,tuple(zf.open(f'{line[0]}/bin_extracted_text.txt').readlines())):
                                                                    found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                                                            case 'PDF':
                                                                if isQueryMatchGewurah(entry_string,tuple(zf.open(f'{line[0]}/pdf_metadata.txt').readlines()),max_line_concat):
                                                                    found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                                                                    continue
                                                                if line[-2] != '0':
                                                                    if isQueryMatchGewurah(entry_string,tuple(zf.open(f'{line[0]}/pdf_extracted_text.txt').readlines()),max_line_concat):
                                                                        found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                                                            case 'DOC':
                                                                if isQueryMatchGewurah(entry_string,tuple(zf.open(f'{line[0]}/doc_metadata.txt').readlines()),max_line_concat):
                                                                    found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                                                                    continue
                                                                if line[-2] != '0':
                                                                    if isQueryMatchGewurah(entry_string,tuple(zf.open(f'{line[0]}/doc_extracted_text.txt').readlines()),max_line_concat):
                                                                        found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                                                            case 'SHP':
                                                                if isQueryMatchChochmah(entry_string,f'_shp_files/{line[0]}.txt',zf):
                                                                    found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                                                            case _:
                                                                pass
                                    elif check_type == 'gdb':
                                        for item in tuple(items):
                                            if not '_gdb/' in item.lower():
                                                items.remove(item)
                                        gdb_items = {}
                                        for item in (items := tuple(items)):
                                            if (gdb_entity := item[:item.find('/')]) in gdb_items.keys():
                                                gdb_items[gdb_entity].append(item[item.find('/')+1:])
                                            else:
                                                gdb_items[gdb_entity] = [item[item.find('/')+1:]]
                                        for gdb_item in tuple(gdb_items.keys()):
                                            if not "%s.%s" % (gdb_item[:gdb_item.rfind('_')],gdb_item[gdb_item.rfind('_')+1:]) in current_grouping:
                                                del gdb_items[gdb_item]
                                        for gdb_item in tuple(gdb_items.keys()):
                                            for item in tuple(gdb_items[gdb_item]):
                                                if isQueryMatchChochmah(entry_string,f"{gdb_item}/{item}",zf):
                                                    found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),gdb_item[:gdb_item.rfind('_')],gdb_item[gdb_item.rfind('_')+1:]))
                                                    break
                        else:
                            for used_name in iterator:
                                current_grouping = grouped_entities[used_name]
                                        ...
                                        with zf.open('_metadata.txt') as tf:
                                            while True:
                                                ...
                                                if not line:
                                                    break
                                                line = tuple(decodeZipTxtLine(line).split('|'))
                                                if "%s.%s" % (line[0][:line[0].rfind("_")],line[0][line[0].rfind("_")+1:]) in grouped_entities[used_name]:
                                                    match line[1]:
                                                        case 'TXT':
                                                            if isQueryMatchGewurah(entry_string,tuple(zf.open(f'_txt_files/{line[0]}.txt').readlines()),max_line_concat):
                                                                found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),line[0][:line[0].rfind('_')],line[0][line[0].rfind('_')+1:]))
                        ...
                        check_type = {item.strip().lower() for item in check_type}
                        if 'docx' in check_type:
                            check_type.remove('docx')
                            check_type.add('doc')
                        for item in self.image_types:
                            if item in check_type:
                                check_type.add('img')
                                check_type.remove(item)
                        for item in ('any','all','every'):
                            if item in check_type:
                                check_type.remove(item)
                        if not len(check_type):
                            if return_tuple:
                        ...
                        for used_name in iterator:
                            current_grouping = grouped_entities[used_name]
                            with ZipFile(f'{self.db_path}/{used_name}.zip') as zf:
                                if '_metadata.txt' in (items := zf.namelist()):
                                    with zf.open('_metadata.txt') as tf:
                                        while True:
                                            line = tf.readline()
                                            if not line:
                                    ...
                                    with open(f'{self.db_path}/_terms_searched/{previous_search}.txt',encoding='utf-8') as tf:
                                        while True:
                                            ...
                                            if not entity:
                                                break
                                            entity = entity.rstrip('\n')
                                            if entity.lower().endswith(f'.{check_type}'):
            ...
            for used_name in iterator:
                ...
                with ZipFile(f'{self.db_path}/{used_name}.zip') as zf:
                    ...
                    for classify in tuple(extracted_data.keys()):
                        if classify == '_txt_files':
                        ...
                        elif classify.lower().endswith('_bin'):
                            ...
                            if 'bin_extracted_text.txt' in extracted_data[classify]:
                                if (txt_file := f'{file_name}/bin_extracted_text.txt') in items:
                                    if not isQueryMatchHessed(entry_string,tuple(zf.open(txt_file).readlines())):
                                        found_matches.remove("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),classify[:-4],classify[-3:]))
                                else:
                                    found_matches.remove("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),classify[:-4],classify[-3:]))
                            ...
                            for txt_file in extracted_data[classify]:
                                if entry_string in getTestName(txt_file):
                    ...
                    if '_alia_dosieroj.txt' in items:
                        with zf.open(f'_alia_dosieroj.txt') as tf:
                            while True:
                                ...
                                if testing_entry_string in (test_name := getTestName(entity)):
                                    found_name_matches.append("%s\\%s" % (self.path_pointer[used_name].replace('/','\\'),entity))
                                for term in terms:
                    ...
                    for classify in tuple(extracted_data.keys()):
                        if classify == '_txt_files':
                                ...
                                if save_found_matches:
                                    for term in terms:
                                        if term.replace('_',' ') in getTestName(txt_file):
                                            term_name_memories[term].append("%s\\%s" % (self.path_pointer[used_name].replace("/","\\"),txt_file))
                            ...
                            if testing_entry_string in getTestName(classify):
                                found_name_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace('/','\\'),classify[:-4],classify[-3:]))
                            if 'bin_extracted_text.txt' in extracted_data[classify]:
                                if isQueryMatchHod(entry_string,(txt_lines := tuple(zf.open(f'{classify}/bin_extracted_text.txt').readlines()))):
                                    found_matches.append("%s\\%s.%s" % (self.path_pointer[used_name].replace("/","\\"),classify[:-4],classify[-3:]))
                                if save_found_matches:
                                    for term in terms:
                                        if term.replace('_',' ') in getTestName(txt_file):
                                            term_name_memories[term].append("%s\\%s.%s" % (self.path_pointer[used_name].replace("/","\\"),classify[:-4],classify[-3:]))
                                        if isQueryMatchHessed(term,txt_lines):
                            ...
                            for txt_file in extracted_data[classify]:
                                if testing_entry_string in (test_name := getTestName(txt_file)):
                                    found_name_matches.append("%s\\%s%s" % (self.path_pointer[used_name].replace("/","\\"),txt_file[:txt_file.rfind(".")],txt_file[txt_file.rfind("."):]))
                                if save_found_matches:
                                    for term in terms:
    ...
    def findAllDuplicates(self, include_other_entities : bool = False, return_tuple : bool = False, save_to_file : bool = False, output_file_type : str = 'excel', output_location : str | None = None, output_name : str | None = None, overwrite_existing_output : bool = False, csv_field_size_limit : int = 131_072, csv_delimiter : str = ',', terminal_progress_display : bool = False) -> None | tuple[tuple]:
        ...
        two_part_line_metadata = {'PDF','BIN','DOC'}
        ...
        type_checker = {"TXT":set(),"BIN":set(),"IMG":set(),"SHP":set(),"DOC":set(),"PDF":set(),"GDB":set()}
                    ...
                    with zf.open('_metadata.txt') as tf:
                        while True:
                            ...
                            if not line:
                                break
                            line = decodeZipTxtLine(line).split('|')
                            if line[1] in two_part_line_metadata:
                    ...
                    with zf.open("_metadata.txt") as tf:
                        while True:
                            ...
                            if entity[1] in current_entities.keys():
                                if entity[1] in two_part_line_metadata:
                            ...
                            match current_entity_type:
                                ...
                                case 'BIN':
                                    ...
                                    if nums[b][:nums[b].find('|')] == '0':
                                        ...
                                        current_lines.append(tuple([decodeZipTxtLine(line) for line in tuple(zf.open(f'{items[c]}/bin_extracted_text.txt').readlines())]))
                                        current_line_count.append(len(current_lines[0]))
                                    if nums[b][nums[b].find('|')+1:] == '0':
                                        current_lines.append(None)
                                        current_line_count.append(0)
                                    else:
                                        current_lines.append(tuple([decodeZipTxtLine(line) for line in tuple(zf.open(f'{items[c]}/bin_nums.txt').readlines())]))
                                    ...
                                    for d in range(c+1,num_items):
                                        ...
                                        if nums[b][:nums[b].find('|')] == '0':
                                            ...
                                            other_lines.append(tuple([decodeZipTxtLine(line) for line in tuple(zf.open(f'{items[d]}/bin_extracted_text.txt').readlines())]))
                                            other_line_count.append(len(other_lines[0]))
                                        if nums[b][nums[b].find('|')+1:] == '0':
                                            other_lines.append(None)
                                            other_line_count.append(0)
                                        else:
                                            other_lines.append(tuple([decodeZipTxtLine(line) for line in tuple(zf.open(f'{items[d]}/bin_nums.txt').readlines())]))
                                        ...
                                        if duplicate_match:
                                            ...
                                            checked.add(f'{current_db_name}|{items[d]}')
                                    for d in range(a+1,num_dbs):
                                        if not (other_db_name := db_names[d]) in type_checker['BIN'] or not other_db_name in line_num_checker[f'{current_line_count[0]}|{current_line_count[1]}|BIN']:
                                            continue
                                        relevant_entities = []
                                        with ZipFile(f'{self.db_path}/{other_db_name}.zip') as zf2:
                                            if not '_metadata.txt' in set(zf2.namelist()):
                                            ...
                                            for relevant_entity in (relevant_entities := tuple(relevant_entities)):
                                                ...
                                                if current_line_count[0]:
                                                    other_lines = tuple([decodeZipTxtLine(line) for line in tuple(zf2.open(f"{relevant_entity}/bin_extracted_text.txt").readlines())])
                                                ...
                                                if current_line_count[1]:
                                                    other_lines = tuple([decodeZipTxtLine(line) for line in tuple(zf2.open(f"{relevant_entity}/bin_nums.txt").readlines())])
    ...
    def getTotalRefSize(self, check_type : str | tuple[str] | list[str] | set[str] = 'any', include_other_entities : bool = False, digi_info_unit : str = "bytes", relevant_decimals : int = 8, rounding : bool = False, return_integer : bool = False, terminal_progress_display : bool = False) -> int | float:
        '''
        ...
        def totalRefSizeCal(num : Decimal, divisor : int, return_integer : bool, rounding : bool, relevant_decimals : int) -> int | float:
            if return_integer:
                return int(num / Decimal(divisor))
            elif rounding:
                return round(float(total_size / Decimal(divisor)),relevant_decimals)
            else:
                return float(total_size / Decimal(divisor))

        if len((digi_info_unit := digi_info_unit.strip())) > 3:
        ...
        match digi_info_unit:
            ...
            case 'megabyte' | 'megabytes' | 'MB':
                return totalRefSizeCal(total_size,1_000_000,return_integer,rounding,relevant_decimals)
            case 'mebibyte' | 'mebibytes' | 'MiB':
                return totalRefSizeCal(total_size,1_048_576,return_integer,rounding,relevant_decimals)
            case 'gigabyte' | 'gigabytes' | 'GB':
                return totalRefSizeCal(total_size,1_000_000_000,return_integer,rounding,relevant_decimals)
            case 'gibibyte' | 'gibibytes' | 'GiB':
                return totalRefSizeCal(total_size,1_073_741_824,return_integer,rounding,relevant_decimals)
            case 'terabyte' | 'terabytes' | 'TB':
                return totalRefSizeCal(total_size,1_000_000_000_000,return_integer,rounding,relevant_decimals)
            case 'tebibyte' | 'tebibytes' | 'TiB':
                return totalRefSizeCal(total_size,1_099_511_627_776,return_integer,rounding,relevant_decimals)
            case 'petabyte' | 'petabytes' | 'PB':
                return totalRefSizeCal(total_size,1_000_000_000_000_000,return_integer,rounding,relevant_decimals)
            case 'pebibyte' | 'pebibytes' | 'PiB':
                return totalRefSizeCal(total_size,1_125_899_906_842_624,return_integer,rounding,relevant_decimals)
            case 'exabyte' | 'exabytes' | 'EB':
                return totalRefSizeCal(total_size,1_000_000_000_000_000_000,return_integer,rounding,relevant_decimals)
            case 'exbibyte' | 'exbibytes' | 'EiB':
                return totalRefSizeCal(total_size,1_152_921_504_606_846_976,return_integer,rounding,relevant_decimals)
            case 'zettabyte' | 'zettabytes' | 'ZB':
                return totalRefSizeCal(total_size,1_000_000_000_000_000_000_000,return_integer,rounding,relevant_decimals)
            case 'zebibyte' | 'zebibytes' | 'ZiB':
                return totalRefSizeCal(total_size,1_180_591_620_717_411_303_424,return_integer,rounding,relevant_decimals)
            case 'yottabyte' | 'yottabytes' | 'YB':
                return totalRefSizeCal(total_size,1_000_000_000_000_000_000_000_000,return_integer,rounding,relevant_decimals)
            case 'yobibyte' | 'yobibytes' | 'YiB':
                return totalRefSizeCal(total_size,1_208_925_819_614_629_174_706_176,return_integer,rounding,relevant_decimals)
            case 'ronnabyte' | 'ronnabytes' | 'RB':
                return totalRefSizeCal(total_size,1_000_000_000_000_000_000_000_000_000,return_integer,rounding,relevant_decimals)
            case 'robibyte' | 'robibytes' | 'RiB':
                return totalRefSizeCal(total_size,1_237_940_039_285_380_274_899_124_224,return_integer,rounding,relevant_decimals)
            case 'quettabyte' | 'quettabytes' | 'QB':
                return totalRefSizeCal(total_size,1_000_000_000_000_000_000_000_000_000_000,return_integer,rounding,relevant_decimals)
            case 'quebibyte' | 'quebibytes' | 'qubibyte' | 'qubibytes' | 'QiB':
                return totalRefSizeCal(total_size,1_267_650_600_228_229_401_496_703_205_376,return_integer,rounding,relevant_decimals)
            case 'bit' | 'bits' | 'b':
                if return_integer:
            ...
            case _:
                ...
                if return_integer:
                    ...
                    return round(float(total_size),relevant_decimals)
                else:
